...
The Settings page now asks only for a cloud login password. The frontend server proxies sync requests to the separate Go backend, keeping the backend URL and bearer token out of browser storage and bundles. Copy `playground/.env.example` to `playground/.env` for local Nuxt development, and set the same cloud variables on the **frontend** Vercel project. Set the variables from `backend/.env.example` on the separate **backend** Vercel project. The token must match in both projects. `BUDGETYAR_ANALYSIS_TOKEN` is not used by the current code.

...
const auth = useAuth()
const { currentUser, isAuthInitialized, initAuth, login, register, updateProfile, changePassword, deleteAccount, logout } = auth
...
.account-card { width: min(100%, 480px); padding: 24px; }
...
.account-tabs button[aria-selected="true"] { border-color: var(--primary); background: rgba(42,168,154,.15); }
...
.account-profile { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 14px; border: 1px solid var(--line); border-radius: 12px; }
.account-profile span { color: var(--muted); }
